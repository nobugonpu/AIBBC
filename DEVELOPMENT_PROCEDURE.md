...
> このセクションはすべての編集・レビューに対して常に適用される。
> 変更・削除は禁止。追記はアーキテクト承認を必須とする。

**A. 章外内容を見つけた時の処理**
- 守備範囲外の記述は削除せず、担当章へ移動する
- 重複する場合は元章に `[内容]は §N を参照` の 1 行だけ残し、正本を 1 箇所に集約する

**B. 章間参照の書式（固定テンプレート）**
- 他章の内容に言及するときは以下のテンプレートのみ使い、本文を重複させない
...
| 変更影響 | `変更影響判定は §12 を参照` |
| 手順 | `手順の詳細は §N を参照` |
| 型・契約 | `型定義は shared/contracts を参照` |

- 参照先が変わった場合は参照元の 1 行だけ直せばよい状態を保つ

**C. 各章冒頭のフォーマット（3 行固定）**
- 各章は本文の先頭に以下の 3 行を固定する
- 「主」は最も直接依存する章、「補」はあわせて読むと理解が深まる章
...
> **関連章**: 主→§2（モジュール分割の前提），補→§4（Warp モデル選択）
...
> **関連章**: 主→§5（合否基準），補→§6（E2E シナリオ），§1（スタック前提）
...
> **関連章**: 主→§4（Warp 運用），補→§5（合否基準），§12（変更影響チェック）
...
> **関連章**: 主→§2（モジュール境界），補→§3（フェーズ別タイミング）
...
> **関連章**: 主→§6（E2E シナリオ），補→§7（Tauri 固有確認），§12（変更影響チェック）
...
> **関連章**: 主→§9（監査証跡），補→§7（Tauri 確認），§5（合格基準）
...
> **関連章**: 主→§11（障害時安全動作），補→§9（監査証跡），§6（E2E シナリオ）
...
> **関連章**: 主→§10（AI フェイルセーフ），補→§8（DB 変更），§5（合格基準）
...
> **関連章**: 主→§5（合格基準），補→§6（E2E），§8（DB 変更），§11（障害時動作）
...
- 2026-04-22 v6: 文書管理ルールを恒久運用原則に格上げ。参照テンプレを `[内容]は §N を参照` 形式に固定。全 15 章の関連章を `主→§X，補→§Y` 優先順位形式に統一
